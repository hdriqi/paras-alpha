...
const NewPost = ({ memento }) => {
  ...
  const _setPage = (result) => {
    const clonePageContent = [...pageContent]
    clonePageContent[showCreationModal] = result
    console.log(clonePageContent)
    setPageContent(clonePageContent)
  ...
  return (
    <div id="new-post" className="bg-dark-0 min-h-screen">
      {
        showCreationModal > -1 && (
          // <NewPostText left={_ => setShowCreationModal(-1)} right={(body) => _setPageText(body)} />
          // <NewPostImage 
          //   left={_ => setShowCreationModal(-1)} 
