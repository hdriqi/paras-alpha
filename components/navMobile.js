    ...
    <div id="modal-bg" onClick={(e) => _closeModal(e)} className="fixed inset-0 w-full h-full z-40 p-8 pt-40" style={{
      ...
      <div className="max-w-sm m-auto bg-white shadow-lg rounded-lg">
        ...
        <div className="opacity-0 absolute">
          <input readOnly type="text" value={`http://localhost:3000/posts/${postData.id}`} id="urlLink" />
        </div>
      </div>
    </div>
  )
...
const NavLink = ({ name, href, as, activePage, setIsActive, children }) => {
  return (
    <Link href={href} as={as}>
      <div onClick={() => setIsActive(name)} className={`flex h-full items-center justify-center relative ${activePage === name ? `text-black-1` : `text-black-3`}`}>
        { children }
        {
          activePage === name && (
...
const NavMobile = () => {
  ...
  return (
    <div className="flex h-12 w-full bg-white" style={{
      ...
      <div className="w-1/5">
        <NavLink name="feed" href="/" activePage={activePage} setIsActive={setIsActive}>
      ...
      <div className="w-1/5">
        <NavLink name="hub" href="/hub/recent" activePage={activePage} setIsActive={setIsActive}>
          <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      <div className="w-1/5">
        <NavLink name="me" href="/[username]" as={`/${profile.username}`} activePage={activePage} setIsActive={setIsActive}>
